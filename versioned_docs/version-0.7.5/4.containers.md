...
- Names must begin with an ASCII alphabet (A-Z or a-z) or an underscore (`_`) and not a digit (0-9)
- The succeeding characters can either be:
  - ASCII: A-Z or a-z
  - ASCII Digits: 0-9
  - Underscore (\_)
- The maximum length is 64 characters
- `PRELOAD` and `PARTMAP` cannot be used as entity names. This limitation will be removed in the future
- For file systems that are case insensitive (usually ones on Windows and macOS), "HELLO" and "hello" will represent
